			...
			userManager = {
				isValidUser: (username, password) => {
					...
					if (users[uIndex].password !== password) {
						console.log("Wrong password for username: " + username + " tried with wrong password:" + password);
						return false;
					}
			...
			if (msg.payload.hasOwnProperty("opcuaCommand") && msg.payload.opcuaCommand === "setInit" && msg.payload.users) {
				...
				(async () => {
					try {
						...
						await node.server.initialize();
						if (node.constructDefaultAddressSpace === false) {
							construct_my_address_space(node.server.engine.addressSpace);
						}
						await node.server.start();
